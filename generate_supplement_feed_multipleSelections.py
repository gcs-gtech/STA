...
If customer chose more than one new titles for a single SKU, keep the one with the highest popularity score.
If there is any question, please reach to @effyli
...
def generateSupplementFeed(result_filepath, supplement_feed_filepath, custom_label, custom_label_number):
 ...
 result = result.loc[result['Approved'] == True]
 result = result.loc[result.groupby('Id')['Popularity_score'].idxmax()]
 ...
 with open(supplement_feed_filepath, mode='a') as supplement_feed_file:
   ...
   nlp_writer.writerow(['id', 'title', custom_label_number])
